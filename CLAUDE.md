...
- **La descarga y el mail comparten `readTicketAttachment`.** La ruta de descarga la usa para devolver el archivo como respuesta HTTP y `resendTicket` la usa para armar el adjunto del mail: las dos leen el mismo archivo del disco, así que el día que el placeholder `public/tickets/entrada.png` se reemplace por el PDF definitivo, cambia en un solo lugar y ambos caminos quedan al día solos.
