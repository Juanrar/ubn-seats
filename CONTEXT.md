...
**Selector** (`SeatPicker`, `hooks/useSeatPicker.ts`) — La máquina de estados de la elección, detrás de un seam: `useSeatPicker(venue, occupied)`. Concentra la selección, el tope, el foco lógico, el movimiento del foco real del DOM, el despacho de teclas (flechas, `Enter`, espacio), la selección ya ordenada (`selectedSeats`), el `total` y el rechazo (ver **el rechazo**, más abajo). `PlateaPicker` es sólo layout, `RowBand`/`VenueMap` sólo pintan lo que reciben y `SelectionBar` sólo pinta `seats`, `total` y `rejection`: ninguno ordena, suma ni maneja foco por su cuenta.

**Selección** (`SeatPicker.selectedSeats`) — Las butacas que la persona eligió, ordenadas por fila y después por número, con un **tope** por compra (`Venue.maxSeats`, hoy `MAX_SEATS = 8`). Las ocupadas nunca *entran* a la selección, pero llamar a `toggle` sobre una no es un no-op: dispara un rechazo (ver **el rechazo**).

**Foco lógico** (`SeatPicker.focusedId`) — Cuál butaca tiene el cursor del teclado. Distinto del foco del DOM: mover el foco lógico obliga a mover explícitamente el foco real, y eso lo hace el propio hook con un `pendingFocus` ref más un `useEffect` que busca `data-seat-id` **dentro de `svg[role="grid"]`** — la única fila con rol de grid es la banda activa; el resto de la sala en `VenueMap` es decorativo y no debe capturar ese foco.
...
**Rechazo** (`Rejection`, `hooks/useSeatPicker.ts`) — Lo que pasa cuando `toggle` no puede aplicar el cambio pedido: butaca ya `ocupada`, o tope de compra alcanzado (`tope`, `Venue.maxSeats`). No es un error silencioso — `toggle` corre igual, y en vez de mutar la selección deja un `Rejection { reason, seatId, message, at }` en el estado del hook, que `SelectionBar` anuncia en una región `aria-live="assertive"`. El campo `at` es un contador: cada rechazo nuevo lo incrementa, así una región `aria-live` puede distinguir "el mismo rechazo de antes" de "un rechazo nuevo" aunque el mensaje sea idéntico.

## La fila activa y la regla

**Fila activa** (`RowFocus.activeRow`, `hooks/useRowFocus.ts`) — Cuál fila de la Platea está expandida a tamaño de dedo en `RowBand`. Vive en su propio hook, separado de `useSeatPicker`: elegir una butaca y cambiar de fila son dos decisiones distintas de la persona. Se puede mover de tres formas — arrastrando **la regla**, tocando una fila en `VenueMap`, o llegando a una butaca de otra fila por teclado (`focusSeatRow`, disparado desde `useSeatPicker.onSeatFocus`) — y las tres terminan en el mismo `setActiveRow`, que hace *clamp* a la primera y la última fila de la sala.

**Banda** (`VenueRow`, `RowBand`) — Un arco de butacas ya agrupado en el Recinto (`Venue.rows`), con su franja (`tier`) y su propio `viewBox` recortado a esa fila (`frameRow`, en `lib/venue/`). `RowBand` es el componente que la pinta a tamaño de dedo: el único `<svg role="grid">` de la página, la única superficie donde el click y el teclado hacen algo. No confundir con la fila dibujada chica dentro de `VenueMap`, que es la misma banda pero en su versión decorativa (`aria-hidden`).

**Regla** (`RowRule`) — El control que barre la sala fila por fila: un `input[type=range]` de escenario a fondo, con una marca por fila y una marca más alta en cada arranque de franja (`tierBands`). Es la superficie pensada para el pulgar en un teléfono — ver la sala entera y elegir la fila sin tener que hacer scroll ni pellizcar para hacer zoom. Mover la regla llama a `setActiveRow`, igual que tocar una fila en el mapa o navegar por teclado.
...
- **Sin pinch-zoom propio** sobre un `<g>` — agrega estado de gestos y compite con el zoom nativo del navegador, que ya resuelve el caso. En su lugar, `VenueMap` escala su `viewBox` al ancho disponible (sala entera siempre visible, sin scroll horizontal) y `RowBand` expande sólo la fila activa a tamaño de dedo — ver **la banda** y **la regla**.
